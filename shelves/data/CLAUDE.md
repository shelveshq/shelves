...
- `query.py` — `run_model_query(model, measures, dimensions, filters, ...)` (SHE-56): aggregated rows for an ad-hoc selection, backing the MCP `query_model` tool. Adds NO query logic — it builds a synthetic `ChartSpec` (measures → `rows`, dims → `cols`/`tooltip`) and routes it through the SAME `adapter.fetch()` a chart uses, so field collection, grain, and filter semantics stay identical across DuckDB and Cube by construction. Inline/source-less models raise `NoDataSourceError` (they can't aggregate). Callers validate refs/filters first.
- `domains.py` — parameter domain resolution (SHE-90). `resolve_parameter_domains(params, models_dir=, data_base_dir=)` turns a field-reference `values:` entry into a `Domain` — distinct values for `string`, `{min, max}` for `number`/`date` — across all three backends, and checks each parameter's `default` against it. Owns the ONE normalizer every backend's raw result passes through, plus the inline (JSON-rows) backend; DuckDB and Cube go through `DataSourceAdapter.fetch_domain_values` / `fetch_domain_bounds`. A module-level TTL cache (`DOMAIN_CACHE_TTL = 60s`) in `resolve_field_domain` avoids re-querying the same `(source_type, model_name, field, param_type)` across compiles; `clear_domain_cache()` (exported from `shelves/__init__.py`) forces immediate invalidation — Studio calls it on model-dir file changes, the TTL handles the rest.
...
- **Filter ownership (decided 2026-07):** the Vega-Lite `transform` filter emitted by `shelves/translator/filters.py` is the **semantic reference** for what a `ShelfFilter` means. Adapter push-down (Cube filters, DuckDB `WHERE`) is an **optimization** that must produce the same row set — never a place to invent different semantics. The translator always emits the VL transform (harmless re-filtering of already-filtered rows keeps inline and adapter-backed charts on one code path); if a backend cannot express a filter faithfully, raise a typed error rather than silently approximating. The "raise rather than silently approximate" rule is enforced by `tests/test_filter_parity.py`, which parametrizes every `FilterOperator` member across all four dispatch sites (VL translator, DuckDB, Cube, schema validator) and fails on any empty result or untyped error. Known deviation: DuckDB compares raw (un-truncated) temporal columns — tracked as SHE-73.
- **Environment:** Cube integration requires `CUBE_API_URL` and `CUBE_API_TOKEN` env vars.
- **Adapters return raw; `domains.py` normalizes.** Never sort, de-duplicate, or type-coerce inside an adapter — that's how backends drift.
- **Domain sources are dimensions, never measures.** A measure domain would produce different numbers on different backends.
- **Inline is not in the adapter registry.** Registering it would change `resolve_data`'s `NoDataSourceError` behavior.
