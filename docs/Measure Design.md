...
Supersedes the DSL Specifications and Architecture design docs. Covers the constrained,
realistic set of multi-measure patterns.

---

## Constraints (from Varun)
...
4. Mark defined alongside its measure. Column/row mark propagates as default.
...
Case D is the "stacked layers" you described — the most complex one.
...
Here's where your instinct of "no new keyword" works well. When `rows`
is a list of measure objects, they stack:
...
## Cols-level mark as default

When the single-measure shelf defines a mark, it becomes the default
for all entries on the other shelf:

```yaml
cols:
  measure: week       # Not a measure — this is a dimension
                      # (but see note below about keeping cols simple)
marks: bar            # Top-level default

rows:
  - measure: revenue  # Inherits bar
  - measure: orders
    mark: line        # Overrides to line
  - measure: arpu     # Inherits bar
```

Wait — your example had `columns: measure: sales` where sales is a measure
on the x-axis (like in a scatter plot). Let me handle that case too:

```yaml
cols:
  measure: sales
  mark: circle        # Default mark for all rows

rows:
  - measure: revenue  # Inherits circle → scatter: sales vs revenue
  - measure: orders   # Inherits circle → scatter: sales vs orders
```

But this raises a question: should `cols` (when it's a single field) be
just a string or an object? I think the cleanest answer is:

- **Dimension on cols:** `cols: week` (string, as in Phase 1)
- **Measure on cols:** `cols: sales` (also string — the field resolver
  knows it's a measure from `data.measures`)

The mark default comes from top-level `marks:`, not from the cols block.
This keeps `cols` clean and avoids introducing a different syntax for
"single field with mark" vs "just a single field."
