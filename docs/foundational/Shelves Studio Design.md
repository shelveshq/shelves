# Shelves Studio: Interface Design Document

This document captures the design rationale, architecture, and implementation plan for Shelves Studio — the analyst-facing authoring environment for the Shelves platform.

---

## 1. Problem Statement

Shelves needs an interface that connects three things with minimal friction: a YAML authoring surface, a deterministic translation pipeline, and a rendering layer. The question is what form that interface should take.
...
- Doesn't play to Shelves's code-first, version-controlled identity.
...
- Left pane: Monaco editor with YAML syntax highlighting and Shelves schema validation.
...
- Ships as `pip install shelves-bi[studio]` → `shelves studio` → browser opens. One command.
...
- **Monaco Editor** via `@monaco-editor/loader` — YAML editing with Shelves JSON Schema validation (generated from Pydantic `ChartSpec.model_json_schema()`).
...
`pip install shelves-bi[studio]` → `shelves studio` → browser opens.
...
File associations (`.yaml` opens in Shelves Studio), "Open Recent" menu, OS notifications for long operations, auto-update, `Cmd+,` preferences panel, drag-and-drop.
...
Standalone `@monaco-editor/loader` gives full editor UX (autocomplete, error squiggles, minimap, syntax highlighting) without maintaining a VS Code codebase. The Shelves JSON Schema (generated from Pydantic models) provides domain-specific autocomplete and validation.
...
KAN-103 (Web Application) becomes the business-user-facing hosted version. Shelves Studio is the analyst-facing local tool. They share the same underlying pipeline. When the full web app ships, Shelves Studio becomes its "developer mode" — same Monaco editor, same preview, just hosted rather than local.

### Upgrade Path

The web app (Phase 6) and Shelves Studio coexist as two entry points for different user types:

- **BI Analysts** use Shelves Studio locally. They write YAML, use CLI agents, version-control with git.
...
**Epic:** KAN-204 — Shelves Studio — Local Dev Server + Native App

| Ticket | Summary | Stage |
|--------|---------|-------|
| KAN-205 | FastAPI dev server with file watcher and WebSocket hot-reload | Stage 1 |
| KAN-206 | Monaco editor integration with Shelves YAML schema validation | Stage 1 |
...
| KAN-211 | CLI entry point — Shelves dev command via pyproject.toml | Stage 1 |
