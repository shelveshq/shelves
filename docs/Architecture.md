# Architecture: AI-Native Declarative Visual Analytics Platform
...
- **BI Analysts** write the YAML DSL directly. They think in Tableau's mental model (rows, columns, marks, detail, color) and get version-controlled, reviewable chart definitions without learning Vega-Lite syntax.
- **Business Users** use natural language prompts. The LLM translates their request into the same YAML DSL. The LLM's job is constrained to producing valid DSL — a much smaller, more reliable translation task than generating raw Vega-Lite.
...
The chart DSL is a YAML grammar inspired by Tableau's shelf model (rows, columns, marks, color, detail, filters). It is the intermediate representation between user intent and Vega-Lite specification. See the **Chart DSL Specification** document for the full grammar.
...
- Each measure in a multi-measure chart can have independent mark types, colors, and detail levels
...
The layout DSL defines dashboard structure as nested horizontal and vertical containers — the same mental model as Tableau dashboards. It references chart specs by name and defines filter panes, text blocks, and spacing. See the **Layout DSL Specification** section within the DSL spec document.
...
- Filters, text objects, and chart sheets are all first-class layout components
- The layout tree is a pure structural description — no styling, no data logic
...
|---|---|
| Dual axis (two measures, same axis) | `layer` spec — two marks sharing `x`, separate `y` encodings |
| Synchronized axes | `resolve: { scale: { y: "shared" } }` (default) |
| Independent axes | `resolve: { scale: { y: "independent" } }` |
| Different mark types per axis | Each layer has its own `mark` definition |
...
- Validate the DSL grammar against 5-10 common chart types
- Use inline data (no semantic layer yet)

### Phase 2: Theme Integration
- Create a sample `theme.json` manually (mimicking Figma output)
...
- Add filter panes with cross-chart targeting

### Phase 5: Figma Theme Pipeline
- Set up Figma → export plugin → Style Dictionary → custom formatter
- Validate end-to-end: Figma change → regenerated theme → updated dashboard
...
- Implement state management for dashboard sessions
