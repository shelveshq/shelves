...
**Compile-time options resolution:** at compose time, filter option sets are resolved from the data layer — sorted distinct values for categorical modes (`multi`, `single`, and `wildcard`, which uses the domain to power a typeahead), and `[min, max]` bounds for quantitative/temporal modes (`range`, `at_least`, `at_most`, `after`, `before`). Options are cached with the same 60-second TTL as parameter domains. If a field has more than 500 distinct values, the list is truncated to 500 and a warning is emitted — a `wildcard` field is high-cardinality by nature, so its list is usually this truncated top-matches prefix, which is fine for a typeahead. If resolution fails (e.g., missing data source), options fall back to `null` with a warning and `wildcard` degrades to a free-text box.

**Placeholder rendering:** each filter emits an HTML placeholder `<div>` with `data-*` attributes (`data-field`, `data-model`, `data-mode`, `data-operator`, `data-targets`, `data-default`, `data-options`). Categorical modes — including `wildcard` — emit `data-options` as a JSON array of `{value, label}` objects; bounds modes emit a `[min, max]` pair. In Studio, `wildcard` renders a Tom Select single typeahead over that domain (committing `contains` on selection/Enter), falling back to a debounced free-text box when the library is absent or no options resolve.
